            ...
            try:
                ...
                if r.status_code == 200:
                    img = pygame.image.load(
                        io.BytesIO(r.content)).convert_alpha()
...
class NeuralVisualizer:
    ...
    def _draw_dashed_rect(self, screen, color, rect, width, dash_length):
        ...
        for i in range(x, x+w, dash_length*2):
            pts.append(((i, y), (min(i+dash_length, x+w), y)))
        for i in range(x, x+w, dash_length*2):
            pts.append(((i, y+h), (min(i+dash_length, x+w), y+h)))
        for i in range(y, y+h, dash_length*2):
            pts.append(((x, i), (x, min(i+dash_length, y+h))))
        for i in range(y, y+h, dash_length*2):
            pts.append(((x+w, i), (x+w, min(i+dash_length, y+h))))
        for p1, p2 in pts:
    ...
    def _run_loop(self):
        ...
        subtitle_font = pygame.font.SysFont(
            "Segoe UI,Arial,sans-serif", 24, bold=False)
        ...
        while self.running:
                ...
                # handle older pygame bindings safely
                    ...
                    if clip_data:
                        if clip_data["type"] == "text":
                            ...
                            try:
                                import memory_vault
                                memory_vault.index_data(
                                    clip_data["content"], "clipboard_text")
                            except:
                                pass
            ...
            if self.sphere_pulse_overclock > 1.0:
                self.sphere_pulse_overclock += (1.0 -
                                                self.sphere_pulse_overclock) * 0.1
            ...
            if self.state == "STANDBY":
                ...
                import state as _st
                try:
                    is_busy = pygame.mixer.get_busy() if hasattr(
                        pygame.mixer, 'get_busy') else False
                except Exception:
                    is_busy = False
                amp_obj = _st.current_volume
                amp = amp_obj.value if hasattr(
                    amp_obj, 'value') else float(amp_obj or 0.0)
            ...
            NODE_COLOR = (int(current_node_r), int(
                current_node_g), int(current_node_b))
            LINE_COLOR = (int(current_line_r), int(
                current_line_g), int(current_line_b))

            # Slow down interpolation (0.05) for "liquid" movement
            self.current_radius += (self.target_radius -
                                    self.current_radius) * 0.05
    ...
    def draw_wing(self, screen, font, mono_font):
        ...
        if not self.context_cards:
            ...
            drop_rect = pygame.Rect(
                wing_x + card_margin, self.height//2 - drop_h//2, wing_w - 2 * card_margin, drop_h)
            self._draw_dashed_rect(screen, (80, 90, 100), drop_rect, 2, 8)
            # drop shadow text
            drop_text1_s = font.render("Drop file to analyze", True, (0, 0, 0))
            drop_text1 = font.render(
                "Drop file to analyze", True, (200, 200, 200))
        ...
        for card in self.context_cards:
            if card.card_type == "WEB":
                ...
                block_h = len(lines) * \
                    target_font.get_linesize() + 2 * card_margin
                card_rect = pygame.Rect(
                    wing_x + card_margin, current_y, wing_w - 2 * card_margin, block_h)
                ...
                if card.status == "running":
                    ...
                    pygame.draw.rect(wave_surf, (0, 212, 255, alpha),
                                     wave_surf.get_rect(), width=2, border_radius=8)
                ...
                if getattr(card, "favicon_surf", None):
                    screen.blit(card.favicon_surf, (wing_x +
                                card_margin + 8, current_y + 8))
                    text_x_offset = 48
                else:
                    text_x_offset = 12

                for i, line in enumerate(lines):
                    txt_surf = target_font.render(
                        line, True, (200, 200, 200) if i > 3 else (150, 200, 255))
                    screen.blit(txt_surf, (wing_x + card_margin + text_x_offset if i < 3 else wing_x +
                                card_margin * 2, current_y + card_margin + i * target_font.get_linesize()))

                if card_rect.collidepoint(pygame.mouse.get_pos()):
                    tools_text_s = font.render(
                        "[ Trash ]   [ Extract ]   [ Sandbox ]", True, (0, 0, 0))
                    tools_text = font.render(
                        "[ Trash ]   [ Extract ]   [ Sandbox ]", True, (255, 255, 255))
                ...
                img_h = int((wing_w - 2*card_margin) *
                            (card.surface.get_height() / card.surface.get_width()))
                img_surf = pygame.transform.smoothscale(
                    card.surface, (wing_w - 2*card_margin, img_h))

                if card.label == "Apex Border":
                    pygame.draw.rect(screen, (0, 212, 255), (wing_x + card_margin - 2,
                                     current_y - 2, wing_w - 2 * card_margin + 4, img_h + 4), border_radius=4)

                screen.blit(img_surf, (wing_x + card_margin, current_y))

                card_rect = pygame.Rect(
                    wing_x + card_margin, current_y, wing_w - 2 * card_margin, img_h)
                if card_rect.collidepoint(pygame.mouse.get_pos()):
                    # Command Deck Hover Element
                    tools_text_s = font.render(
                        "[ Trash ]   [ Analyze ]   [ Sandbox ]", True, (0, 0, 0))
                    tools_text = font.render(
                        "[ Trash ]   [ Analyze ]   [ Sandbox ]", True, (255, 255, 255))
                    t_x = card_rect.right - tools_text.get_width() - 8
                    t_y = card_rect.bottom - tools_text.get_height() - 8
                    screen.blit(tools_text_s, (t_x+2, t_y+2))
                    screen.blit(tools_text, (t_x, t_y))
                ...
                block_h = len(lines) * \
                    target_font.get_linesize() + 2*card_margin
                pygame.draw.rect(screen, (20, 25, 30), (wing_x + card_margin,
                                 current_y, wing_w - 2*card_margin, block_h), border_radius=8)

                for i, line in enumerate(lines):
                    txt_surf = target_font.render(line, True, (200, 200, 200))
                    screen.blit(txt_surf, (wing_x + 2*card_margin, current_y +
                                card_margin + i*target_font.get_linesize()))
        ...
        if self.wing_open_ratio > 0.5:
            ...
            screen.blit(scan_surf, (wing_x, scan_y_abs - 1),
